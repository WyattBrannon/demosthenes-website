...
// Demosthenes member.bundle.js vJSON-2025-08-16-3
(function(){
  var dbg = null, header = null;
  function qs(id){ return document.getElementById(id); }
  function setDbg(html){ if(dbg){ dbg.innerHTML = html; } }
  function setErr(html){ if(dbg){ dbg.innerHTML = '<span class="err">'+html+'</span>'; } if(header && /Loading|Fetching/.test(header.textContent)){ header.textContent='Error — see message above.'; } }
  ...
  function seatText(chamber, state, district){ var c=(chamber||"").toLowerCase(); return (c==="house"||c==="rep") ? "("+(state||"??")+"-"+(((district===""||district==="0"||district==null)?"AL":pad2(district)))+")" : "("+(state||"??")+")"; }
  ...
  function boot(){
    ...
    var bid = params.get('bioguide') || "";
    var url = DB + "/members/" + bid + ".json";
    setDbg("<code>DATA_BASE</code> = " + DB + " · <code>bioguide</code> = " + (bid||"(missing)") + " · <code>URL</code> = " + (bid?url:"(n/a)"));

    if(!bid){ header.textContent="Missing ?bioguide=…"; return; }

    header.textContent = "Fetching member JSON…";
    fetch(url, {cache:"no-store"}).then(function(r){
      if(!r.ok){ header.textContent="Failed to load member JSON."; setErr("HTTP "+r.status+" for <code>"+url+"</code>"); return Promise.reject(); }
      return r.json();
    }).then(function(data){
      if(!data || !data.identity || !data.alignment){ header.textContent="Invalid member JSON."; setErr("JSON missing identity/alignment"); return; }
