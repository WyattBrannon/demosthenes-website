...
try { window.__memberData = data; } catch(e){}
      ;(function(){
        ...
        function placePersonalList(txt){
          var items = [];
          if (typeof txt === 'string') {
            items = txt.split(';').map(function(s){ return s.trim(); }).filter(function(s){ return s.length > 0; });
          }
          ['personal-info','adv-card-personal'].forEach(function(id){
            var sec = document.getElementById(id);
            if(!sec) return;
            var title = sec.querySelector('.section-title');
            if(!title) return;
            var lid = (id==='personal-info') ? 'personal-list' : 'adv-personal-list';
            // If list already inserted, remove & rebuild to apply new formatting
            var existing = sec.querySelector('#'+lid);
            if (existing) existing.remove();
            var none = sec.querySelector('#personal-none');
            if (items.length === 0){
            ...
            ul.style.fontSize = '1.06em';           // slightly larger
            ul.style.lineHeight = '1.45';           // a little more spacing
            items.forEach(function(t, idx){
              var li = document.createElement('li');
              var txtItem = capFirst(t);
              if (idx < items.length - 1) {
                if (!/;\s*$/.test(txtItem)) txtItem += ';';
              } else {
                // Last item: ensure no trailing semicolon(s)
                txtItem = txtItem.replace(/;\s*$/,'');
              }
              li.textContent = txtItem;
              li.style.marginBottom = '4px';       // spacing between lines
              ul.appendChild(li);
            });
            title.insertAdjacentElement('afterend', ul);
          });
        }
        var txt = (data && typeof data.profileText === 'string') ? data.profileText.trim() : '';
        placePersonalList(txt);
      })();
