...
(function(){"use strict";
  ...
  function renderDial(container, valuePct, centerText, label, tooltip, strokeColor){ 
    ...
    wrap.innerHTML=''
      ...
      +   '<text x="50%" y="52%" text-anchor="middle" font-size="20" font-weight="800">'+centerText+'</text>'
      + '</svg>'
      + '<div class="label">'+label+'</div>';
    container.appendChild(wrap);
  }

  // Fallback name via legislators-current.yaml (if needed)
  function fetchNameFromYAML(DB, bioguide){ 
    var url=DB+'/legislators-current.yaml';
    return fetch(url,{cache:'no-store'}).then(function(r){if(!r.ok) return null; return r.text();}).then(function(txt){ 
      if(!txt) return null;
      var idx=txt.indexOf('bioguide: '+bioguide); if(idx<0) idx=txt.indexOf("bioguide: '"+bioguide+"'"); if(idx<0) idx=txt.indexOf('bioguide: "'+bioguide+'"'); if(idx<0) return null;
      var start=txt.lastIndexOf('\n- ',idx); if(start<0) start=Math.max(0,idx-200);
      var snippet=txt.slice(start,idx+400);
      var m=snippet.match(/official_full:\s*["']?([^"\n]+)["']?/); if(m&&m[1]) return m[1].trim();
      var f=(snippet.match(/first:\s*['"]?([^"\n]+)['"]?/)||[])[1];
